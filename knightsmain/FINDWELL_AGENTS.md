...
| `config.findwell.example.yaml` | Configuration template |
| `findwell/cli.py` | Primary CLI entry point (`findwell` / `researchclaw`) |
| `findwell/pipeline/executor.py` | Stage execution logic |
| `findwell/pipeline/runner.py` | Pipeline orchestration |
| `findwell/experiment/validator.py` | Code validation (AST, security, imports) |
| `findwell/experiment/visualize.py` | Chart generation |
| `findwell_extension/` | FindWell Chrome Extension (Manifest V3, 3 API Keys, 4-LLM Failover) |
| `findwell_app/` | FindWell Flutter Mobile App (iOS & Android) |
