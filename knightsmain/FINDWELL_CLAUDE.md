...
├── cli.py                   # CLI: `findwell` / `researchclaw` commands
├── pipeline/
...
├── llm/
│   └── client.py            # LLMClient (OpenAI-compatible), multi-provider failover
├── experiment/
...
researchclaw/
└── __init__.py              # Alias package redirecting imports to findwell for backward compatibility

findwell_extension/          # Manifest V3 Chrome Extension (3 API Keys, 4-LLM Failover Engine)
findwell_app/                # Flutter Mobile Application (iOS & Android)
docs/
└── supabase_schema.sql      # Supabase PostgreSQL schema with RLS policies & Vector search
```
