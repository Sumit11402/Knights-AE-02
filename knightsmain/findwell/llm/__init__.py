...
def create_llm_client(config: RCConfig) -> LLMClient | ACPClient:
    ...
    - ``"ensemble"`` / ``"multi"`` → :class:`ConsensusLLMClient` (4-LLM consensus system)
    ...
    - ``"groq"`` → :class:`LLMClient` with Groq base URL
    - ``"gemini"`` → :class:`LLMClient` with Gemini OpenAI-compatible base URL
    ...
    if config.llm.provider in ("ensemble", "multi"):
        from researchclaw.llm.ensemble import ConsensusLLMClient as _Consensus
        return _Consensus.from_env()
