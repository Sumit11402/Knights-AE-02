...
2. Use bold (**text**) for important terms.`;
  ...
  async function loadCloudHistory() {
    ...
    try {
      const url = `${SUPABASE_URL}/rest/v1/conversations?user_id=eq.${userId}&select=*,messages(*)&order=updated_at.desc&limit=10`;
      const res = await fetch(url, {
        headers: {
          'apikey': SUPABASE_ANON_KEY,
          'Authorization': `Bearer ${currentToken || SUPABASE_ANON_KEY}`
      ...
      if (res.ok) {
        ...
        return data.map(item => ({
          id: item.id,
          page: { title: item.title || 'Research Chat', url: 'https://findwell.ai/chat' },
  ...
  async function syncSessionToCloud() {
    ...
    try {
      ...
      const convTitle = currentSession.page?.title || 'Extension Research Chat';
